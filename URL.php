...
class URL
{
    ...
    */
    function set_relative($url)
    {
        ...
        // 2009-12-30 Fix by Dominik Deobald: Leave here if only query is set to allow relative urls like "?a=b" without any other components.
		if (($parts['path'] == '') && !isset($parts['scheme']) && !isset($parts['host'])) {
            return;
        }
        if (isset($parts['host'])) {
...
/*
$u = new URL('http://www.actiontrip.com/index/cheats.phtml?name=&platform=pc');
$u->set_relative('?name=0-9&platform=pc');
echo $u->as_string();
*/
?>
